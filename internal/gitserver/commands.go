...
func (c *clientImplementor) MergeBase(ctx context.Context, repo api.RepoName, base, head api.CommitID) (_ api.CommitID, err error) {
	ctx, _, endObservation := c.operations.mergeBase.With(ctx, &err, observation.Args{
		MetricLabelValues: []string{c.scope},
		Attrs: []attribute.KeyValue{
			attribute.String("base", string(base)),
			attribute.String("head", string(head)),
		},
	})
	defer endObservation(1, observation.Args{})
